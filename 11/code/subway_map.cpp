...
public:
    ...
    Line() {name = "NULL";}
    Line(std::string name, std::vector<Node*> nodes, int id) : id(id) {
        this->name = name;
        this->nodes = nodes;
    }
    std::string getName(){
...
class Graph{
    ...
    struct AbstGraph{
        ...
        void allocate(const std::vector<int>& ids, int lineNum) {
            ...
            for (int id : ids) {
                int newid = id;
                if ((int)changeSeqList[id].size() != 0) {//allocate a new node
        ...
        std::pair<int, std::vector<std::pair<int, int>>> run(int s, int t) {
            ...
            while (complete == false) {
                ...
                for (int j=_last[pos]; j!=-1; j=_next[j]) {
                    if (from[y[j]] == -1) {
                        ...
                        if (y[j] < (int)mapper.size() && mapper[y[j]] == mapper[t]) {
                            t = y[j];
                            complete = true;
                            break;
                        }
            ...
            while (t >= (int)mapper.size() || mapper[t]!=mapper[s]) {
                t = from[t];
                route.emplace_back(t);
            }
            ...
            for (int abst_id : route) {
                if (abst_id >=(int) mapper.size()) continue;
        ...
        std::pair<int ,std::vector<v2_route>> run_v2(int s, int t, int time) {
            ...
            std::vector<int> route_backbone;
            for (int abst_id : route) {
                if (get_v2id_revid(abst_id) >= (int)mapper.size()) continue;
                route_backbone.push_back(abst_id);
            }
            //deal wrong start and end point
        ...
        std::pair<int, std::vector<std::pair<int,int>>> find_circuit(int start, int change_cost) {
           ...
           */   
            ...
            for (int i=0; i<(int)min_result.size(); i++) {
                if ((int)ans.size() == 0 || ans.back().first!=mapper[min_result[i]]) {
...
//            infile.imbue(std::locale("zh_CN.GBK"));
    ...
    void build(const Config* config) {
        ...
        for (int lineNum = 0; lineNum < (int)lines.size(); ++lineNum) {
...
//        puts(t.c_str());
    ...
    std::pair<std::pair<sv, sv>, sv> run_query_from_a_to_b_v2(std::string s, std::string t, int time){
        ...
        for (int i=0; i<(int)route.size();i++) {
            ...
            auto calculateTime = [&](int t) {
                ...
                if ((int)sh.size() < 2) sh = "0"+sh;
                if ((int)sm.size() < 2) sm = "0"+sm;
                return  sh + ":" + sm;
            };
            ...
            if (route[i].line != lst_line) {
                ...
                if (i+1 <(int) route.size() && route[i].node == route[i+1].node) {
                    int j = i+1;
                    while (j+1 <(int) route.size() && route[j].node == route[j+1].node) j++;
    ...
    std::pair<sv, sv> run_query_random_walk(std::string startNodeName, const Config* config){
        ...
        for (int i=0;i<(int)routeNode.size();i++) {
            // std::cout << routeNode[i] << std::endl;
            if (routeNode[i] == startNodeName) {
                for (int j=i, cnt = 0; cnt<(int)routeNode.size(); cnt++, j=(j+1)%routeNode.size()) {
