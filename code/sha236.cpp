...
public:
    std::string mycalSHA236(std::string path)
    {
        ...
        unsigned int *Kp, *W, *M, a, b, c, d, e, f, g, h, T1,
        ...
        if (!(ch = (char*)calloc(b + 256, 1))) {
            // 256 == 64 * sizeof(int), it's for W.
            fclose(fp);
            return 0;
        }
        rewind(fp);
        ...
        char buff[200];
        snprintf(buff, sizeof(buff), "%.8x%.8x%.8x%.8x%.8x%.8x%.8x%.8x", H0, H1, H2, H3, H4, H5, H6, H7);
        std::string SHA256 = buff;
